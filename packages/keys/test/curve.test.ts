...
describe('Bytes are on the curve', () => {
    it('returns false when a public key does not lie on the Ed25519 curve', () => {
        expect(isOnCurve(OFF_CURVE_KEY_BYTES[0])).to.be.false;
        expect(isOnCurve(OFF_CURVE_KEY_BYTES[1])).to.be.false;
    });

    it('returns true when a public key lies on the Ed25519 curve', () => {
        expect(isOnCurve(ON_CURVE_KEY_BYTES[0])).to.be.true;
        expect(isOnCurve(ON_CURVE_KEY_BYTES[1])).to.be.true;
    });
});