...
describe('Derivation', () => {
    ...
    describe('master key', () => {
        it('should derive master key', () => {
            // Reference: https://github.com/satoshilabs/slips/blob/master/slip-0010.md#test-vector-1-for-ed25519

            const seed = Buffer.from("000102030405060708090a0b0c0d0e0f", "hex");
            const descriptor = Derive.masterKey(seed);

            expect(Buffer.from(descriptor.key).toString('hex')).to.eq("2b4be7f19ee27bbf30c667b642d5f4aa69fd169872f8fc3059c08ebae2eb19e7");
            expect(Buffer.from(descriptor.chain).toString('hex')).to.eq("90046a93de5380a72b5e45010748567d5ea02bbf6522f979e05c0d8d8ca9fffb");
        });
    });


    describe('derive key (vector 1)', () => {
        const seed = Buffer.from("000102030405060708090a0b0c0d0e0f", "hex");
        const data = [
            {
                path: "m/0'",
        ...
        for (const item of data) {
            ...
            const child = Derive.path(seed, path);
            const keypair = child.toKeypair();

            (async () => {
                it(`should derive key for ${item.path}`, () => {
                    expect(path.toString()).to.eq(item.path);
                    expect(Buffer.from(child.chain).toString('hex')).to.eq(item.chainCode);
                    expect(Buffer.from(child.key).toString('hex')).to.eq(item.key);
                    expect(keypair.getPublicKey().toBuffer().toString('hex')).to.eq(item.publicKey);
                });
            })();
        }
    });

    describe('derive key (vector 2)', () => {
